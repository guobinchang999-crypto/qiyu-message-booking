...
import type { Appointment, AppointmentAuditRecord, BookingOptionPayload, BookingStatus, BusinessReportRow, CheckinTask, CouponCampaign, CustomerProfile, MemberAccount, RoomResource, SelectOption, ServiceCatalogItem, ServiceOrder, StoreProfile, TherapistProfile } from '@/types';
...
export const adminRemoteApi = {
  ...
  async getRegionOptions(): Promise<SelectOption[]> {
    const rows = await adminRequest<RemoteCatalogOption[]>('/catalog/options/regions');
