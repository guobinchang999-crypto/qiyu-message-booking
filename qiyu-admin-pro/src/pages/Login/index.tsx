...
export default function LoginPage() {
  ...
  const onFinish = async (values: { identifier: string; credential: string; remember: boolean }) => {
    ...
    try {
      ...
      locked.current = false; setLoading(false);
    }
  };
  return (
    <div className="login-page">
      <div className="login-intro">
        <div className="login-brand-mark">栖</div>
        <Typography.Title level={1}>栖愈运营中心</Typography.Title>
        <Typography.Paragraph>统一品牌门店运营 · 预约履约 · 资源管理</Typography.Paragraph>
        <div className="login-intro-card">今日服务的每一次安心体验，都在这里被妥善安排。</div>
      </div>
      <Card className="login-card" variant="borderless">
        <Typography.Title level={3}>管理员登录</Typography.Title>
        <Typography.Paragraph type="secondary">登录后开始今日门店工作。</Typography.Paragraph>
        <Form layout="vertical" form={form} initialValues={{ remember: true }} onFinish={onFinish}>
          ...
          <Button type="primary" htmlType="submit" size="large" block loading={loading}>进入运营中心</Button>
        </Form>
