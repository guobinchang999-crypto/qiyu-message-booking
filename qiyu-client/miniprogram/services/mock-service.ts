...
export const mockService: BookingService = {
  ...
  login: (mobile:string, code:string):Promise<LoginPayload> => code === loginCopy.demoCode ? delay({ token:`mock-token-${Date.now()}`, expiresIn:7200, user:{ mobile, name:'林知夏', avatarText:'林' } }) : Promise.reject(new Error('验证码不正确')),
