...
const resolveDraft = (draft: BookingDraft) => {
  ...
  const therapist = draft.therapistMode === 'auto' ? undefined : therapists.find((item) => item.id === draft.therapistId) || therapists[0];
  const slot = timeSlots.find((item) => item.id === draft.slotId) || timeSlots[2];
  return { store, service, therapist, slot };
};
const buildPayment = (draft: BookingDraft): PaymentSummary => {
...
const buildConfirmation = (draft: BookingDraft): BookingConfirmationPayload => {
...
const buildRebookDraft = (booking: Booking): BookingRebookPayload => ({
...
const paginate = <T>(items: T[], page = 1, pageSize = 2) => {
...
const buildRescheduleDraft = (booking: Booking): BookingReschedulePayload => ({
  sourceBookingId: booking.id,
  draft: {
    storeId: booking.store.id,
    serviceId: booking.service.id,
...
const orderDictionaries = {
  ...
  tabs: [
    { key:'all', label:'全部' },
    { key:'pending_payment', label:'待支付', statuses:['PENDING_PAYMENT'] as BookingStatus[] },
    { key:'arriving', label:'待到店', statuses:['BOOKED', 'CHECKED_IN'] as BookingStatus[] },
    { key:'in_service', label:'服务中', statuses:['WAITING_SERVICE', 'IN_SERVICE', 'PENDING_SETTLEMENT'] as BookingStatus[] },
    { key:'completed', label:'已完成', statuses:['COMPLETED'] as BookingStatus[] },
    { key:'cancelled', label:'已取消', statuses:['CANCELLED'] as BookingStatus[] }
  ] as OrderTabItem[],
...
const serviceDictionaries = {
  pageTitle: '服务项目',
  allCategory: '全部',
  categories: ['全部', ...Array.from(new Set(services.map((service) => service.category)))],
  storeSwitchLabel: '静安寺店 ›',
  searchPlaceholder: '搜索肩颈、推拿、SPA',
  clearSearchText: '清除',
  sortOptions: [
    { key: 'recommended' as const, label: '推荐优先' },
    { key: 'sales' as const, label: '销量优先' },
    { key: 'price' as const, label: '会员价优先' }
  ],
  cardMeta: {
    durationUnit: '分钟',
    servedPrefix: '已服务',
...
const storeDetailDictionaries = {
  ...
  tabs: [
    { key:'services' as const, label:'项目' },
    { key:'therapists' as const, label:'技师' },
...
const buildDateLabels = (): string[] => Array.from({ length: 4 }, (_, index) => {
  const date = new Date();
...
const buildDateValues = (): string[] => Array.from({ length: 4 }, (_, index) => {
  const date = new Date();
  date.setDate(date.getDate() + index + 1);
...
const timeDictionaries = {
  ...
  periods: [{ key:'MORNING' as const, label:'上午' }, { key:'AFTERNOON' as const, label:'下午' }, { key:'EVENING' as const, label:'晚上' }],
  defaultPeriod: 'AFTERNOON' as const,
  statusLabel: {
    available: '可预约',
    limited: '即将约满',
    full: '已约满'
  ...
  legend: [
    { status: 'available' as const, label: '可预约' },
    { status: 'limited' as const, label: '即将约满' },
    { status: 'full' as const, label: '已约满' }
  ]
};
const therapistDictionaries = {
  pageTitle: '选择技师',
  modes: [
    { key: 'specified' as const, label: '指定技师' },
    { key: 'auto' as const, label: '系统自动分配' }
  ],
...
const pageStateDictionaries = {
  ...
  stores: {
    ...
    sortOptions: [
      { key: 'frequent' as const, label: '常去优先' },
      { key: 'distance' as const, label: '距离优先' },
      { key: 'rating' as const, label: '评分优先' }
    ],
    cardMeta: {
      ratingUnit: '分',
      nextAvailablePrefix: '最近可约'
...
export const mockService: BookingService = {
  getHome: ():Promise<HomePayload> => delay({ frequentStores:stores.filter((store) => store.isFrequent), nearbyStores:stores.filter((store) => !store.isFrequent), featuredServices:services, featuredTherapists:therapists.slice(0,2), copy:homeCopy }),
  getStores: () => delay(stores),
  getServices: () => delay(services),
  getStore: (id) => delay(stores.find((store) => store.id === id) || stores[0]),
  getService: (id) => delay(services.find((service) => service.id === id) || services[0]),
  getTherapists: () => delay(therapists),
  getTimeSlots: () => delay(timeSlots),
  ...
  sendLoginCode: (mobile:string):Promise<LoginCodePayload> => delay({ mobile, requestId:`LOGIN-${Date.now()}`, expiresIn:60, verificationCode:loginCopy.demoCode }),
  login: (mobile:string, code:string):Promise<LoginPayload> => code === loginCopy.demoCode ? delay({ accessToken:`mock-token-${Date.now()}`, tokenType:'Bearer', expiresIn:7200, principal:{ userId:'mock-customer-1', userType:'CUSTOMER', displayName:'林知夏', avatarText:'林', roles:['CUSTOMER'], permissions:['booking:read','booking:create','booking:update','review:create'], storeScopes:[{ scopeType:'SELF', storeIds:[] }] } }) : Promise.reject(new Error('验证码不正确')),
  ...
  createBooking: (draft:BookingDraft) => { const confirmation = buildConfirmation(draft); const code = nextBookingCode(); const booking:Booking = { ...initialBooking, id:`booking-${Date.now()}`, code, qrImageUrl:buildQrImageUrl(code), store:confirmation.store, service:confirmation.service, therapist:confirmation.therapist || therapists[0], scheduledAt:confirmation.scheduledAt, contact:`${draft.contact} 138****1288`, payment:confirmation.payment }; bookings = [booking, ...bookings]; return delay(booking); },
  rescheduleBooking: (draft:BookingDraft) => { const confirmation = buildConfirmation(draft); bookings = bookings.map((booking) => booking.id === draft.sourceBookingId ? { ...booking, store:confirmation.store, service:confirmation.service, therapist:confirmation.therapist || booking.therapist, scheduledAt:confirmation.scheduledAt } : booking); return delay(bookings.find((booking) => booking.id === draft.sourceBookingId) || bookings[0]); },
