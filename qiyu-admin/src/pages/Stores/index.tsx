...
export default function StoresPage() {
  ...
  return <div className="qiyu-page">
    ...
    <Modal title={editing ? '编辑门店' : '新增门店'} open={open} confirmLoading={saving} onOk={submit} onCancel={() => setOpen(false)} width={640} destroyOnHidden>
      <Form form={form} layout="vertical" preserve={false} style={{ marginTop: 24 }} initialValues={initialValues}>
        <Row gutter={16}><Col span={12}><Form.Item name="code" label="门店编码" rules={[{ required: true }, { pattern: /^[A-Za-z0-9_-]{2,32}$/, message: '请输入 2-32 位字母、数字、下划线或短横线' }]}><Input disabled={!!editing} placeholder="例如 JING_AN" /></Form.Item></Col><Col span={12}><Form.Item name="name" label="门店名称" rules={[{ required: true, message: '请输入门店名称' }]}><Input /></Form.Item></Col></Row>
        <Row gutter={16}><Col span={12}><Form.Item name="phone" label="联系电话"><Input /></Form.Item></Col><Col span={12}><Form.Item name="businessHours" label="营业时间" rules={[{ required: true }, { pattern: /^\d{2}:\d{2}-\d{2}:\d{2}$/, message: '格式应为 HH:mm-HH:mm' }]}><Input /></Form.Item></Col></Row>
        <Row gutter={16}><Col span={8}><Form.Item name="province" label="省份"><Input /></Form.Item></Col><Col span={8}><Form.Item name="city" label="城市"><Input /></Form.Item></Col><Col span={8}><Form.Item name="district" label="区县"><Input /></Form.Item></Col></Row>
        <Form.Item name="address" label="详细地址" rules={[{ required: true, message: '请输入详细地址' }]}><Input /></Form.Item>
        <Row gutter={16}><Col span={8}><Form.Item name="longitude" label="经度"><InputNumber min={-180} max={180} precision={6} style={{ width: '100%' }} /></Form.Item></Col><Col span={8}><Form.Item name="latitude" label="纬度"><InputNumber min={-90} max={90} precision={6} style={{ width: '100%' }} /></Form.Item></Col><Col span={8}><Form.Item name="regionId" label="区域 ID"><InputNumber min={1} precision={0} style={{ width: '100%' }} /></Form.Item></Col></Row>
        <Row gutter={16}><Col span={8}><Form.Item name="status" label="营业状态" rules={[{ required: true }]}><Select options={[{ label: '营业中', value: '营业中' }, { label: '休息中', value: '休息中' }]} /></Form.Item></Col><Col span={8}><Form.Item name="rating" label="门店评分"><InputNumber min={0} max={5} precision={1} style={{ width: '100%' }} /></Form.Item></Col><Col span={8}><Form.Item name="sortOrder" label="展示顺序"><InputNumber min={0} precision={0} style={{ width: '100%' }} /></Form.Item></Col></Row>
