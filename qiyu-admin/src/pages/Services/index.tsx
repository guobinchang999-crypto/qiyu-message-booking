...
export default function ServicesPage() {
  ...
  return <div className="qiyu-page">
    ...
    <Modal title={editing ? '编辑服务项目' : '新增服务项目'} open={open} confirmLoading={saving} onOk={submit} onCancel={() => setOpen(false)} width={640} destroyOnHidden><Form form={form} layout="vertical" preserve={false} initialValues={initialValues} style={{ marginTop: 24 }}>
      <Row gutter={16}><Col span={12}><Form.Item name="code" label="项目编码" rules={[{ required: true }]}><Input disabled={!!editing} /></Form.Item></Col><Col span={12}><Form.Item name="name" label="项目名称" rules={[{ required: true }]}><Input /></Form.Item></Col></Row>
      <Row gutter={16}><Col span={12}><Form.Item name="category" label="服务分类" rules={[{ required: true }]}><Input /></Form.Item></Col><Col span={12}><Form.Item name="status" label="发布状态" rules={[{ required: true }]}><Select options={[{ label: '上架', value: '上架' }, { label: '下架', value: '下架' }]} /></Form.Item></Col></Row>
      <Row gutter={16}><Col span={8}><Form.Item name="durationMinutes" label="服务时长（分钟）" rules={[{ required: true }]}><InputNumber min={10} step={10} style={{ width: '100%' }} /></Form.Item></Col><Col span={8}><Form.Item name="preparationMinutes" label="准备时间（分钟）" rules={[{ required: true }]}><InputNumber min={0} style={{ width: '100%' }} /></Form.Item></Col><Col span={8}><Form.Item name="cleanupMinutes" label="清洁时间（分钟）" rules={[{ required: true }]}><InputNumber min={0} style={{ width: '100%' }} /></Form.Item></Col></Row>
      <Row gutter={16}><Col span={12}><Form.Item name="price" label="标准价" rules={[{ required: true }]}><InputNumber min={0} precision={2} prefix="¥" style={{ width: '100%' }} /></Form.Item></Col><Col span={12}><Form.Item name="memberPrice" label="会员价" rules={[{ required: true }]}><InputNumber min={0} precision={2} prefix="¥" style={{ width: '100%' }} /></Form.Item></Col></Row>
