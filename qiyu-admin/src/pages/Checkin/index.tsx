...
export default function CheckinPage() {
  ...
  const startService = async (record: CheckinTask) => {
    await adminRemoteApi.transitionAppointment(record.id, 'start-service');
    await reload();
    message.success('服务已开始');
  };
  const batchCheckIn = async () => {
    await Promise.all(selectedRowKeys.map((key) => adminRemoteApi.transitionAppointment(String(key), 'checkin')));
    setSelectedRowKeys([]);
    await reload();
  ...
  const columns: ColumnsType<CheckinTask> = [
    ...
    { title: '核销', key: 'checkin', render: (_, record) => <Space>{can(session, 'booking:checkin') && <Button size="small" type="primary" disabled={record.status !== 'BOOKED'} onClick={() => checkIn(record)}>确认签到</Button>}{can(session, 'booking:update') && <Button size="small" disabled={record.status !== 'CHECKED_IN'} onClick={() => startService(record)}>开始服务</Button>}</Space> }
  ];
