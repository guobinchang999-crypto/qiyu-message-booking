...
import { Button, Card, Col, DatePicker, Form, Input, InputNumber, Modal, Popconfirm, Row, Select, Space, Table, Tag, message } from 'antd';
...
export default function CouponsPage() {
  ...
  return <div className="qiyu-page"><div className="qiyu-page-header"><div><h1 className="qiyu-page-title">优惠券</h1><div className="qiyu-page-description">维护全门店通用优惠券模板、有效期与投放状态。</div></div>{manageable && <Button type="primary" icon={<PlusOutlined />} onClick={() => showForm()}>创建优惠券</Button>}</div>
    ...
    <Modal title={editing ? '编辑优惠券' : '创建优惠券'} open={open} confirmLoading={saving} onOk={submit} onCancel={() => setOpen(false)} width={640} destroyOnHidden><Form form={form} layout="vertical" preserve={false} initialValues={initialValues} style={{ marginTop: 24 }}>
      <Row gutter={16}><Col span={12}><Form.Item name="code" label="优惠券编码" rules={[{ required: true }]}><Input disabled={!!editing} /></Form.Item></Col><Col span={12}><Form.Item name="name" label="优惠券名称" rules={[{ required: true }]}><Input /></Form.Item></Col></Row>
      <Row gutter={16}><Col span={12}><Form.Item name="discountType" label="优惠类型" rules={[{ required: true }]}><Select options={[{ label: '固定金额', value: 'FIXED' }, { label: '折扣比例', value: 'PERCENT' }]} /></Form.Item></Col><Col span={12}>{discountType === 'PERCENT' ? <Form.Item name="discountPercent" label="折扣比例（%）" rules={[{ required: true }]}><InputNumber min={0.01} max={100} precision={2} style={{ width: '100%' }} /></Form.Item> : <Form.Item name="discountAmount" label="优惠金额" rules={[{ required: true }]}><InputNumber min={0.01} precision={2} prefix="¥" style={{ width: '100%' }} /></Form.Item>}</Col></Row>
      <Row gutter={16}><Col span={12}><Form.Item name="thresholdAmount" label="使用门槛" rules={[{ required: true }]}><InputNumber min={0} precision={2} prefix="¥" style={{ width: '100%' }} /></Form.Item></Col><Col span={12}><Form.Item name="status" label="投放状态" rules={[{ required: true }]}><Select options={Object.entries(statusMeta).map(([value, meta]) => ({ value, label: meta.label }))} /></Form.Item></Col></Row>
