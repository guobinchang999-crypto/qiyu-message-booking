...
public final class MockCatalogMapper {
    ...
    public static Store store(Map<String, Object> row) { return new Store(text(row,"id"), text(row,"name"), text(row,"address"), text(row,"phone"), decimal(row,"latitude"), decimal(row,"longitude"), text(row,"distance"), decimal(row,"rating"), text(row,"businessStatusCode"), text(row,"businessStatusLabel"), text(row,"nextAvailableAt"), text(row,"businessHours"), bool(row.get("frequent")), text(row,"coverImageUrl")); }
    ...
    public static Therapist therapist(Map<String, Object> row) { return new Therapist(text(row,"id"), text(row,"name"), text(row,"storeId"), text(row,"level"), decimal(row,"rating"), integer(row,"experienceYears"), strings(row.get("skills")), number(row,"extraFee"), text(row,"nextAvailable"), text(row,"status"), text(row,"statusLabel"), text(row,"avatarUrl"), text(row,"portraitUrl"), text(row,"introduction")); }
